...
if len(sys.argv) < 4:
    print ("usage: %s res.txt lab.lab res_wer.out N_spk "%(sys.argv[0]));
    print ("usage: %s res/target-seg_gmm.res data/evaluate.lab res.wer 200 "%(sys.argv[0]));
    sys.exit(0);
...
fp_out.write("wav_name\tres_spk\tres_score\tlab_spk\n");
...
for line in fp_res:
    ...
    if num_spk == N_spk:
        num_all_wav += 1;
        ### 正确
        if spk_ok in spk_max:
            num_all_wav_ok += 1;
        else:
            num_all_wav_err += 1;
        fp_out.write("%s\t%s\t%.3f\t%s\n"%(wav, spk_max[0], score_max[0], spk_ok));
        
        num_spk = 0;
        spk_max = ["xxx", 'xxx', 'xxx'];   ## 记录目前为止 识别最好的那个spk是啥
        score_max = [0,0,0,0];

fp_out.write("num_all_wav:%d\n"%(num_all_wav));
fp_out.write("num_all_wav_ok:%d\n"%(num_all_wav_ok));
fp_out.write("num_all_wav_err:%d\n"%(num_all_wav_err));
